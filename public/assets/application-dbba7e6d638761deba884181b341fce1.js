 ...
 * Webflow: Polyfills
 */
Webflow.define('webflow-polyfills', function ($, _) {
  'use strict';
  ...
  api.ready = function () {
    if (doc.documentElement.className.indexOf('w-ie8') >= 0) {
      fixBgSize();
    }
  };

  // Fix background-size in IE8
  // See https://github.com/louisremi/background-size-polyfill
  function fixBgSize() {
    // For each Webflow stylesheet, inject behavior into styles that have background-size
    _.each(doc.styleSheets, function(sheet) {
      var href = sheet.href;
      var match = 'webflow.css';
      
      if (href.indexOf(match, href.length - match.length) < 0) {
        return;
      }

      // Recursively walk the CSS tree and add polyfill behavior
      eachRule(sheet, function(rule) {
        var style = rule.style || null;
        var css = style ? style.cssText : null;
        if (css && css.toLowerCase().indexOf('background-size:') > 0 && !style.behavior) {
          style.behavior = 'url(/js/backgroundsize.min.htc)';
        }
      });
    });
  }

  function eachRule(obj, cb) {
    if (!obj) return;

    var rules = obj.cssRules || obj.rules;
    if (rules) {
      _.each(rules, function(r) { eachRule(r, cb); });
